/* ============================================================================
   ...
   МЕРИТЬ НАДО ПОЛНОЙ ГОНКОЙ, А НЕ ОДНИМ КРУГОМ (v1.15.68). До этого пробник гонял
   один круг, и это молча предполагало, что поле едет кучей: за круг быстрый болид
   успевал доехать до головы только потому, что голова была рядом. Когда поле
   намеренно разъехалось (v1.15.66: Сильверстоун P1→P22 1.05 -> 3.89 с на круг,
   то есть соперники растянулись по трассе вчетверо дальше), пробник стал мерить
   «успел ли догнать», а не «смог ли обогнать», и валился на исправной игре:
   12-13 мест из 20 при пороге 14 на всех настройках, какие ни поставь.
   Полной гонкой на той же сборке выходит 19-20 мест из 20 — то есть болид
   выигрывает гонку с последнего места, как и должен.
   ЗУБЫ ПРОВЕРЕНЫ на архивной v1.15.44 ИМЕННО В ЭТОЙ МЕРКЕ: 5.0 / 5.7 / 5.7 мест,
   то есть порог 14 отделяет исправную игру от сломанной с запасом вчетверо.
   Это ровно тот урок, что записан после v1.15.59: обгоны ИИ мерить полной гонкой.
...
const FLICKER = 600;             // НА КРУГ: с v1.15.68 гонка длиннее одного круга, и счёт делится на их число — иначе порог, снятый за круг, тихо утраивается
...
function run(opt) {
  ...
  const laps = opt.laps === undefined ? 3 : +opt.laps;   // полная гонка: см. шапку
  ...
  for (const T of H.tracks(true)) {
    ...
    for (const boost of [2, 20]) {
      ...
      const flick = Math.round(Math.max(...rows.map(x => x.flicker)) / Math.max(1, laps));   // дребезг считается НА КРУГ
