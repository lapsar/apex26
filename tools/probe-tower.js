...
function run(opt) {
  ...
  /* ШЕСТЬ зёрен, а не три (v1.15.69). На Монреале мельтешение стоит вплотную к порогу
     и на трёх зёрнах прыгает через него от шума: одна и та же сборка дала 5.7 на
     [7,91,13] и 4.0 на шести, а отданная v1.15.65 — 5.0 на обоих счётах, то есть
     ровно на пороге. Пробник, который переключается между «прошёл» и «не прошёл»
     от выбора зёрен, не сторожит ничего. Порог 5 при этом НЕ трогали. */
