...
function buildSceneryObject(THREE,o,mats,helpers){
  ...
  // ЛИЦО ПО ТОЧКЕ (faceLatLon). Для трибуны у своего участка «наружу от полотна»
  // и есть направление взгляда, и якоря хватает. Но у трибуны, стоящей ДАЛЕКО
  // от того, на что она смотрит, якорь не работает вовсе: buildScenery
  // пересчитывает atS из latLon (защита «трибуна не должна уехать на 100 м»),
  // и вместо апекса шпильки в якорь попадает ближайшая точка петли — нога
  // подхода. Трибуну 34 из-за этого развернуло лицом на подход, а не на
  // шпильку; владелец увидел это на устройстве (v1.15.33). Ключ задаёт
  // направление явно: зрители смотрят В указанную точку, пандус поднимается ОТ неё.
  if(o.faceLatLon){
    const [fx,fz]=helpers.toXZ(o.faceLatLon[0],o.faceLatLon[1]);
    out.set(x-fx,0,z-fz).normalize();
    dir.set(-out.z,0,out.x);                                  // ширина — поперёк взгляда
  }
