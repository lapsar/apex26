/* ============================================================================
   ...
   Игрок стоит примерно на 11-й позиции и не трогается вообще. За 25 с все
   соперники позади него обязаны объехать неподвижный болид и уйти вперёд.
   Сколько их — считается от фактического места на решётке, а не от 11:
   время игрока вставляется в уже посчитанную квалификацию, и на части зёрен
   он встаёт девятым или десятым.
...
function run(opt) {
  ...
  for (const T of H.tracks(true)) {                    // видимые в меню: гоночные пробники долгие
    ...
    for (const seed of seeds) {
      ...
      const gp = H.startRaceAt(env, 11);          // просимая позиция достигается не всегда: время игрока вставляется в уже посчитанную квалификацию
      H.noRetirements(env);
      H.lightsOut(env);
      ...
      // сверяется не «ровно 11», а то, что расстановка по dist согласована с местом на решётке:
      // на зерне 11 игрок вставал 10-м, соперников позади было 12, и жёсткая единица ловила исправную гонку
      if (res.behind !== 21 - gp) r.fail(`${name} / зерно ${seed}: позади игрока ${res.behind} соперников,`
        + ` а по месту на решётке (P${gp}) должно быть ${21 - gp}`);
    }
  }
