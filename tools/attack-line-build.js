...
const ATKLIM = +arg('atklim', '0');     // коридор для ЗАЩЁЛКНУВШЕГО атаку: обычные 0.62 полуширины не пускают его внутрь
...
/* 2-bis. КОРИДОР ДЛЯ АТАКУЮЩЕГО. Главное, что нашёл замер: внутрь не пролезть.
      Обороняющийся в повороте стоит на своей линии в 0.42 полуширины от осевой,
      ограничитель полосы стоит на 0.62 — значит внутри него остаётся 0.20 полуширины,
      то есть 1.4 м на Монреале вместо нужных 2.6. Поэтому «сторона внутрь» сама по себе
      смещение вбок не увеличивает, а УМЕНЬШАЕТ (56 % -> 42 %).
      Ключ раздвигает коридор ТОЛЬКО тому, кто защёлкнул атаку, и не дальше жёсткого
      предела 0.75, за которым болид уже выпускается за кромку. */
if (ATKLIM > 0) {
  sub(/    const lim=hw\*0\.62;want=Math\.max\(-lim,Math\.min\(lim,want\)\);/,
`    const lim=hw*((c.duel&&!c.ovLock)?${ATKLIM}:0.62);want=Math.max(-lim,Math.min(lim,want));`);
}
