...
// Montreal 43 -> 42 (v1.15.28): размеченные трибуны выключили трибуны-заглушки
// (два меша, сиденья и козырёк) и добавили один свой — сиденья; каркас трибун
// рисуется тем же материалом, что и пит-билдинг. В гонке 492 -> 491.
// Майами открыта в меню в v1.15.83: 40 мешей — обобщённое окружение плюс меш
// перемычек барьера, появившийся вместе с защитой от складывания.
// v1.15.84: 40 -> 42. Частичная разметка — пит-билдинг и две трибуны, обмеренные
// по снимку; трибуны-заглушки при этом выключились, а зоны вылета остались
// обобщёнными (проверка hasRunoff раздельная). Вызовов в гонке 491 при потолке 495.
// v1.15.87: Miami 42 -> 43. Щиты торможения у поворотов 1, 11 и 17 — полотно щита
// рисуется своим материалом (board) и потому даёт отдельный меш; стойки идут общим
// материалом с каркасами. Плата — один вызов отрисовки: в гонке 491 -> 492 при
// потолке 495, ровно как на Монреале в v1.15.23.
// v1.15.95: Miami 43 -> 44. Стадиону дан свой цвет стен (серо-бежевый вместо общего
// серо-синего) — новый материал стоит ровно одного меша. Остекление лишнего меша
// НЕ стоит: его серо-голубой совпал с уже имеющимся материалом каркаса трибун,
// и строитель переиспользует его. В гонке 493 вызова при потолке 495.
// Майами 44 -> 45 в v1.15.96: остекление стадиона получило свой синий цвет и перестало
// совпадать с материалом каркаса трибун. В гонке это 494 вызова при потолке 495 —
// столько же, сколько у Сильверстоуна (495) все эти сборки.
const SCENE_BUDGET = { Monza: 43, Silverstone: 46, Montreal: 42, Miami: 45 };
