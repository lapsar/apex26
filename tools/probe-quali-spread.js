...
function run(opt) {
  ...
  /* Полоса пересмотрена в v1.15.68 вместе с физикой, а не подогнана под неё.
     Прежние 25…45 % были сняты, когда поле на Сильверстоуне/Профи укладывалось
     в 1.05 с и случайные ±0.2 % (≈0.21 с) свободно тасовали голову решётки.
     С единым законом сцепления поле там разъехалось до 3.89 с, и тасовать голову
     разбросом стало нечем: сильнейший берёт поул в 79.9 % (Монца, поле 2.46 с — 63.1 %).
     Это не «разброс исчез» — сам разброс проверяется выше и по-прежнему ровно ±0.2 %
     и симметричен. Это значит, что сильнейший пилот на широком поле и должен обычно
     быть на поуле. Лотерею сторожат две проверки НАД этой строкой (слабый в топ-6
     и лидер ниже P11, обе обязаны быть нулевыми) — они от ширины поля не зависят.
     Полоса 50…95 %: ниже — разброс снова решает больше, чем разница пилотов;
     100 % значило бы, что квалификация перестала быть событием вовсе. */
  if (key.poleToBest < 0.50 || key.poleToBest > 0.95)
    r.fail(`Silverstone/Профи: сильнейший берёт поул в ${pc1(key.poleToBest)} квалификаций, ожидалось 50…95 %`
      + ' (ниже — решётка снова лотерея, выше — квалификация перестала быть событием)');
