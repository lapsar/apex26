/* ============================================================================
   ...
   Времена соперников в квалификации считаются qualiLapTime (это estLapTime,
   ускоренная на QUALI_PACE — квалификационный круг быстрее гоночного темпа,
   как в жизни) и умножаются на
...
function sample(trackIdx, diff, draws, seed) {
  ...
  const clean = env.evalIn(`(function(){
    ...
    qualiField.forEach(function(r){
      /* Формула зовётся ПО ИМЕНИ, а не повторяется здесь: с v1.15.79 квалификационный
         круг быстрее гоночного темпа на QUALI_PACE, и пробник, считавший базу через
         estLapTime, принимал этот подъём за перекос разброса (замер: −0.94…−0.55 %
         вместо ±0.20). Тот же урок, на котором семь сборок врала справка lap-potential. */
      o[r.name] = qualiLapTime(r.skill, sel.diff);
    });
    return o;
  })();`);
